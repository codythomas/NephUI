...
NephUI.Keybinds = NephUI.Keybinds or {}
local Keybinds = NephUI.Keybinds
...
local function IsKeybindEnabledForAnyViewer()
    ...
    for _, viewerSettingName in pairs(viewersSettingKey) do
        ...
        if NephUI.db.profile[enabledKey] then
            return true
        end
    end
    return false
...
local function GetKeybindSettings(viewerSettingName)
    ...
    local fontName = NephUI.db.profile["cooldownManager_keybindFontName_" .. viewerSettingName]
    if not fontName or fontName == "" then
        fontName = NephUI.db.profile.cooldownManager_keybindFontName or defaults.fontName
    end

    local fontColor = NephUI.db.profile["cooldownManager_keybindFontColor_" .. viewerSettingName]
    if not fontColor then
        fontColor = NephUI.db.profile.cooldownManager_keybindFontColor or defaults.fontColor
    end

    return {
        anchor = NephUI.db.profile["cooldownManager_keybindAnchor_" .. viewerSettingName] or defaults.anchor,
        fontSize = NephUI.db.profile["cooldownManager_keybindFontSize_" .. viewerSettingName] or defaults.fontSize,
        offsetX = NephUI.db.profile["cooldownManager_keybindOffsetX_" .. viewerSettingName] or defaults.offsetX,
        offsetY = NephUI.db.profile["cooldownManager_keybindOffsetY_" .. viewerSettingName] or defaults.offsetY,
        fontColor = fontColor,
        fontName = fontName,
    }
end
...
local function ApplyKeybindTextSettings(icon, viewerSettingName)
    ...
    local fontFlags = NephUI.db.profile.cooldownManager_keybindFontFlags or {}
...
local function InjectCachedDataOntoIcons()
    ...
    PrintDebug("[NephUI Keybinds] Injected cached data onto", injectedCount, "icons")
...
local function UpdateIconKeybind(icon, viewerSettingName)
    ...
    local enabledKey = "cooldownManager_showKeybinds_" .. viewerSettingName
    if not NephUI.db.profile[enabledKey] then
...
eventFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    if event == "EDIT_MODE_LAYOUTS_UPDATED" then
        PrintDebug("[NephUI Keybinds] EditMode layout changed - rebuilding cache")
    ...
    elseif event == "PLAYER_ENTERING_WORLD" then
        ...
        Keybinds:UpdateAllKeybinds()
        PrintDebug(
            "[NephUI Keybinds] PLAYER_ENTERING_WORLD - LoadOrBuild result:",
            "inLockdown:",
            tostring(InCombatLockdown())
        )
    elseif
...
function Keybinds:Shutdown()
    PrintDebug("[NephUI Keybinds] Shutting down module")
    ...
    if NephUI.db and NephUI.db.profile and NephUI.db.profile.keybindCache then
        NephUI.db.profile.keybindCache = nil

        PrintDebug("[NephUI Keybinds] Wiped DB cache")
    end
...
function Keybinds:Enable()
    ...
    PrintDebug("[NephUI Keybinds] Enabling module")
        ...
        for viewerName, settingName in pairs(viewersSettingKey) do
            ...
            if viewerFrame then
                hooksecurefunc(viewerFrame, "RefreshLayout", function()
                    ...
                    PrintDebug("[NephUI Keybinds] RefreshLayout called for viewer:", viewerName)
...
function Keybinds:Disable()
    ...
    PrintDebug("[NephUI Keybinds] Disabling module")
...
function Keybinds:Initialize()
    if not IsKeybindEnabledForAnyViewer() then
        PrintDebug("[NephUI Keybinds] Not initializing - no viewers enabled")
        return
    end

    PrintDebug("[NephUI Keybinds] Initializing module")
