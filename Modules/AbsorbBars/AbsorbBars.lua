...
local function CreateAbsorbBarForUnit(unit)
    ...
    local function UpdateAbsorbBarFrameLevel()
        ...
        if powerBar and powerBar:IsShown() then
            -- Power bar exists and is shown, set absorb to be below it
            absorbBar:SetFrameLevel(powerBar:GetFrameLevel() - 1)
        else
    ...
    local function UpdateAbsorbBarPosition()
            ...
            if db and db[unit] then
                ...
                if PowerBarDB and PowerBarDB.Enabled ~= false then
                    ...
                    if not alternatePowerBarShown then
                        ...
                        powerBarHeight = PowerBarDB.Height or 3
                        -- Add 1 pixel for spacing between power bar and absorb bar
                        powerBarHeight = powerBarHeight + 1
                    end
                end
            end
        end
        ...
        elseif anchorMode == "healthEnd" then
            ...
            -- BOTTOMRIGHT extends to the right edge of health bar
