...
-- Module metadata
UnitFrames.IDENTIFIER = "NephUIUnitFrames"
...
function UnitFrames:InitializeDB()
    -- Prefer NephUI's profile DB if available
    if NephUI and NephUI.db and NephUI.db.profile then
...
function UnitFrames:PixelPerfect(value)
    if NephUI and NephUI.Scale then
...
local function GetSafeHealthPercent(unitToken)
    -- Use ScaleTo100 curve - returns 0-100 directly (matches old NephUI)
...
function UnitFrames:Print(...)
    if NephUI and NephUI.Print then
        NephUI:Print(...)
        return
    end
    print("|cff00ccff[NephUI UF]|r", ...)
end

function UnitFrames:DebugPrint(...)
    if self.devMode then
        print("|cff00ccff[NephUI UF Debug]|r", ...)
    end
end

function UnitFrames:VerbosePrint(...)
    if self.verboseLogging then
        print("|cff888888[NephUI UF]|r", ...)
    end
end
