--[[
    NephUI Unit Frames - Frame Creation
...
function UnitFrames:CreateUnitFrame(unitToken, frameIndex, isRaidFrame)
    ...
    local frameName = isRaidFrame 
        and ("NephUIRaidFrame" .. frameIndex)
        or ("NephUIPartyFrame" .. (frameIndex == 0 and "Player" or frameIndex))
...
function UnitFrames:RegisterAuraWithMasque(icon, auraType)
    ...
    local groupName = auraType == "BUFF" and "NephUI Buffs" or "NephUI Debuffs"
    
    if auraType == "BUFF" then
        if not self.MasqueGroup_Buffs then
            self.MasqueGroup_Buffs = self.Masque:Group("NephUI", "Buffs")
        end
        ...
        if not self.MasqueGroup_Debuffs then
            self.MasqueGroup_Debuffs = self.Masque:Group("NephUI", "Debuffs")
        end
